...
4. Leave `includeDetails=false` for base listings. Enable it for available listing-page descriptions and photos. Independently select `enrichProfiles` for paid seller profile data: the plan-specific price per unique identifiable profile saved per run ($0.75-$1/1,000 by plan), including partial profiles. Leave it off to avoid profile charges.
